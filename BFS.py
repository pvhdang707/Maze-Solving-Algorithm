...
def search(maze):
    ...
    frontier=[]                # create a frontier list_ FIFO
    ...
    if(start !=None and goal!=None)  :                    # flag to indicate if the goal has been found
        while len(frontier) > 0:                           # loop until the frontier list is empty
            cur=frontier[0]                          # current cell equals x and  y positions
            if cur==goal:
...
def write_output(maze, visited, file_path,flag):
    if flag==1:
        ...
        f = open("./output/BFS/" + file_path + "_BFS.txt", "w")
        start=f_start(maze)
        goal=f_goal(maze)
        if visited == None:
            f.write("NO")
            ...
            pygame.image.save(screen, "./output/BFS/" + file_path + "_BFS.jpg")
